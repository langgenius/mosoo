...
const AGENT_BUILDER_LLM_PLANNER_SYSTEM_PROMPT = [
  ...
  "- prompt.",
  "- existing skillIds, mcpServerIds, and environmentId.",
  ...
  "Stage 3 assembly: make the Environment decision (bind, create, or skip), then add optional Skills and MCP servers.",
...
function createAgentBuilderPlannerResponseFormat(): Record<string, unknown> {
  ...
  const nodeSchema = {
    ...
    properties: {
      ...
      summary: { type: "string" },
      targetType: stringEnumSchema(["draft", "environment", "mcp", "skill", "workflow"]),
    },
    required: [
      "actions",
      "askUser",
