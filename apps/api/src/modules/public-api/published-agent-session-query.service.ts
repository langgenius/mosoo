...
import type { PublishedAgentListThreadsResponse } from "@mosoo/contracts/public-api";
import { sessionRunsTable, sessionsTable } from "@mosoo/db";
import { parsePlatformId } from "@mosoo/id";
import type { AccountId, AgentId, PublicThreadId, SessionId } from "@mosoo/id";
...
import { toPublishedSessionSummary } from "./published-agent-api-presenter";
...
export async function admitPublicSessionCaller(
  ...
  const agent = await admitPublishedAgentCaller(database, caller, access.row.agent_id);
...
export async function listPublishedAgentThreads(
  database: D1Database,
  caller: AuthenticatedViewer,
  input: {
    agentId: AgentId;
...
): Promise<PublishedAgentListThreadsResponse> {
  await admitPublishedAgentCaller(database, caller, input.agentId);
  ...
  const rows = await getAppDatabase(database)
    ...
    .limit(PUBLISHED_AGENT_THREADS_MAX_LIMIT)
    .all();
