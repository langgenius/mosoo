...
export async function createSkillFromUpload(
  ...
  await ensureOrganizationMembership(bindings.DB, viewerId, organizationId);
  const published = await publishSkillSnapshot(bindings, organizationId, input);
  ...
  return getSkillSummary(bindings.DB, viewer, skillId);
}
...
export async function updateOwnedSkillPackage(
  ...
  const viewerId = viewer.id;
  const skill = await ensureSkillEditor(bindings.DB, viewerId, skillId);
  const published = await publishSkillSnapshot(bindings, skill.organizationId, input);
  const timestampMs = currentTimestampMs();
