...
const CONTRACT_SCHEMA_SQL = readFileSync(
  new URL("./published-agent-http-core-schema.sql", import.meta.url),
  "utf8",
)
  .concat("\n")
  .concat(
    readFileSync(new URL("./published-agent-http-wechat-schema.sql", import.meta.url), "utf8"),
  );
...
const PUBLIC_API_VENDOR_CREDENTIAL_ID = "vendor-openai-company" as VendorCredentialId;
...
export async function createPublicHttpContractDatabase(): Promise<SqliteD1Database> {
  ...
  await db
    ...
    .values(
      [
        [PUBLIC_API_TEST_IDS.ownerAccount, null, "owner"],
        [PUBLIC_API_TEST_IDS.memberAccount, null],
        [PUBLIC_API_TEST_IDS.collaboratorAccount, null],
        [PUBLIC_API_TEST_IDS.disabledAccount, nowMs],
      ].map(([accountId, disabledAt, role = "member"]) => ({
        accountId,
        createdAt: nowMs,
        disabledAt,
  ...
  const apiKeySecretId = await storeVendorCredentialSecret(
    ...
    {
      actorAccountId: PUBLIC_API_TEST_IDS.ownerAccount,
      apiKey: "sk-test",
  ...
  await db
    ...
    .values({
      ...
      skillsJson: "[]",
      spaceBindingsJson: "[]",
      summary: "Published test version",
      versionNumber: 1,
    })
    .run();
  ...
  await db
    ...
    .values([
      {
        assignedByAccountId: PUBLIC_API_TEST_IDS.ownerAccount,
        createdAt: nowMs,
        resourceId: PUBLIC_API_TEST_IDS.agent,
        resourceType: "agent",
        role: "user",
        targetId: PUBLIC_API_TEST_IDS.collaboratorAccount,
        targetKind: "user",
      },
      {
        assignedByAccountId: PUBLIC_API_TEST_IDS.ownerAccount,
        ...
        resourceType: "agent",
        role: "user",
        targetId: PUBLIC_API_TEST_IDS.organization,
