...
**Rule:** consume tokens via Tailwind theme classes (`text-fg-2`, `bg-paper-200`,
`border-border-strong`, `bg-accent-soft`, etc.). Do not hardcode hex in components.
...
Product transitions stay in the 120–260ms band and convey state, not decoration.
Every animation has a `prefers-reduced-motion` fallback.

## Components

Built on Base UI + Radix, shadcn-style, in `src/shared/ui/`. `button.tsx` (variants:
...
`page-header.tsx`, `dropdown-menu.tsx`, `tooltip.tsx`, `scope-tabs.tsx`, `switch.tsx`,
