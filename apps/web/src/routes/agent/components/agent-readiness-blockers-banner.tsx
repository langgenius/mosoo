...
function readinessBlockMessages(
  ...
  return readiness ? formatReadinessIssueMessages(readiness.issues, t) : [];
}
...
export function AgentReadinessBlockersBanner({
  ...
  const messages = readinessBlockMessages(readiness, t);
  const providerPresentation = getPrimaryProviderReadinessPresentation(readiness.issues, t);
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-destructive text-[12px] font-semibold">
            {t(providerPresentation?.title ?? "agent.setupRequired")}
          </div>
          {hasSummary ? (
        ...
        onRetryProviderCheck !== undefined ? (
          <Button
            ...
            variant="outline"
          >
            {isRetrying ? t("agent.retrying") : t(RETRY_PROVIDER_CHECK_TEXT)}
          </Button>
        ) : null}
      </div>
