    ...
    <TooltipPrimitive.Portal>
      ...
      >
        <TooltipPrimitive.Popup
          ...
          className={cn(
            "relative w-fit origin-(--transform-origin) rounded-sm bg-emphasis px-2.5 py-1.5 text-[12px] text-balance text-emphasis-foreground data-[open]:animate-in data-[open]:fade-in-0 data-[open]:zoom-in-95 data-[closed]:animate-out data-[closed]:fade-out-0 data-[closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
            className,
          )}
          {...props}
        >
          {children}
          <TooltipPrimitive.Arrow className="bg-emphasis fill-emphasis z-50 size-2.5 rotate-45 rounded-xs data-[side=bottom]:top-[-3px] data-[side=left]:right-[-3px] data-[side=right]:left-[-3px] data-[side=top]:bottom-[-3px]" />
        </TooltipPrimitive.Popup>
      </TooltipPrimitive.Positioner>
    </TooltipPrimitive.Portal>
  );
