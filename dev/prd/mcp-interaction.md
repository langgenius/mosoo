...
| **InfoSec / Compliance** | "Employee tokens must be encrypted at rest, and the OAuth hops and callbacks must be auditable."                                                                          |
...
A **personal Server** always uses per-user credentials — the creator holds their own and **cannot share them** (to share, an Admin must recreate it as organization-managed, leaving an audit trail).
...
- Admin deletes an Organization Shared Server → a confirmation dialog + an audit log entry + cascade deletion of credentials (including the Service Account credential).
