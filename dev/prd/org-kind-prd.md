...
> ⚠️ **DEPRECATED — DO NOT IMPLEMENT.** The `organization.kind` axis and the CE Creation Slot model described below were both removed from the product. The `kind` axis was removed, which deleted `apps/api/src/modules/organizations/domain/organization-kind.policy.ts` along with the `kind` field on the GraphQL schema, services, and tests. The remaining "CE allows self-creating at most one ordinary Organization" slot was then removed on 2026-06-10, which deleted `apps/api/src/modules/organizations/domain/organization-creation-slot.policy.ts`, dropped the `organizationCreationSlot` field from the Viewer GraphQL type / contracts / web session, and enabled the `+ Create Organization` entry unconditionally. The current contract is in [Identity & Access](./identity-access.md), which states: "A `personal / team` kind field **does not exist**." Whether an Org is a "company" is now determined entirely by whether it has claimed a primary domain. This document is retained only for history.
