...
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Action                            | An operational action triggered by a single save. This PRD locks 5 config-change tiers: `direct-update` / `restart-process` / `patch-and-restart` / `recreate-preserving-state` / `fork-agent`. `reset-agent-state` is not config-triggered; it is a separate Pet Danger Zone entry point (`RuntimeStateOperationName` in `pkgs/contracts/src/agent/agent.contract.ts`), not one of the 5 tiers. |
