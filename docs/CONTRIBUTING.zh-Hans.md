...
- `bun >= 1.4.0-canary.1`，需要支持文本格式 `bun.lock` 的 `lockfileVersion: 2`。
- `just >= 1.51`
...
- 在仓库依赖安装前 bootstrap 时使用 `bun install --frozen-lockfile`。
...
```bash
git submodule update --init   # 拉取 .skills/mosoo-skills（用 --recurse-submodules 克隆可跳过此步）
...
可复用的编码 agent skills 维护在 [`langgenius/mosoo-skills`](https://github.com/langgenius/mosoo-skills) 仓库，这里以 Git submodule 的方式 vendor 到 `.skills/mosoo-skills`，并通过 `.claude/skills` 软链暴露出来，使任何在本仓库工作的 agent 都能自动发现这些 skills。

- 执行 `git submodule update --init`（或克隆时带 `--recurse-submodules`）后 skills 才会可用；否则 `.claude/skills` 指向空的 submodule 目录。
- 升级到最新 skills：`git submodule update --remote .skills/mosoo-skills`，然后提交更新后的 submodule 指针。
- 不要在 `.skills/mosoo-skills` 下直接修改 skills —— 它们的源头在上游 `mosoo-skills` 仓库。
