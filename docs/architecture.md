...
2. **App Domain**
   ...
   - **Agent and resource ownership**: Agents, Threads / Sessions, Environments, Skills, MCP servers, Provider credentials, Channels, Agent exposure state, App export, app health, logs, and app-scoped cost are App-owned resources. Files Library records belong to the tenant/account file boundary rather than the Agent Manifest.
...
4. **File Service**
   The File Service is the storage boundary for a Files Library plus Session attachments/artifacts:
   - **Abstraction and permission control**: Files Library records belong to the tenant/account boundary and are not an App source tree. App package and draft files remain scoped implementation records for Agent import/export and builder workflows.
...
| **Files Library**            | Pet / Cattle | Explicit long-lived user-managed file library. It is not runtime-writable by default in the MVP.                                                                                                                     | User uploads, shared knowledge files, reusable input files                                               | FILE_BUCKET + `file_record(scope_kind=library)`                          |
