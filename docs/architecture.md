...
The current priority is OPCs, personal developers, and small self-hosted deployments. A user should be able to bring `PRD.md`, invoke `@mosoo`, and get a running Agent App in their own Cloudflare account with low operational overhead. In the current construction phase, assume one human owns one Organization: Organization is the account / billing / tenant shell, App is the code and data boundary, and App is the console noun. Additional operational controls are extension paths for the same architecture, not default complexity for the current community edition.
...
2. **App Service**
   ...
   - **Resource ownership**: Agents, Threads / Sessions, Spaces, Environments, Skills, MCP servers, Provider credentials, Channels, Agent exposure state, App export, app health, logs, and app-scoped cost are App-owned resources. Organization rollups remain for billing.
...
4. **File Service**
   ...
   - **Abstraction and permission control**: Space is an App-owned file asset for current App work. During the single-owner phase, Space access is derived from App owner access; old permission-table language must not be used as a fallback.
