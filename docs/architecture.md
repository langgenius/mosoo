...
Target direction (2026-09-09): [SPEC](./SPEC.md) defines a Project-scoped managed runtime for general application-backend tasks. A Project key, Agent, input, and optional files create a Session that executes one task. New Sessions resolve the latest Agent configuration and retain internal immutable snapshots; callers neither select versions nor deploy Workers per configuration. A task may use multiple model requests and tool calls, but additional user input always creates a new Session. Full-access execution retains isolation and authorization; interactive approvals and Git resource onboarding are deferred. See [remaining slices](./prd/managed-agent-v1.md).

The target reuses checkpoint as the successful task commit and safe-reclamation gate: persist outputs, events, and usage and commit a ready checkpoint before reporting success or recycling the successfully executed task's workspace. Checkpoint failure is explicit. Failed, cancelled, and budget-exhausted tasks retain their actual outcomes and any saved artifacts. Checkpoints do not introduce public continuation, native-conversation restore, or a renewable recovery window. History and saved artifacts remain accessible after runtime reclamation without restoring a conversation. Existing data-retention promises and append-only migration rules remain in force.

The topology and flows below describe current implementation, including legacy Thread continuation and its existing checkpoint retention, until the remaining slices land. They do not reintroduce those behaviors as target acceptance criteria. #581 Project-key authorization is shipped; the single-task lifecycle is not yet shipped.
...
2. **Project Domain**
   Project Domain owns the business, resource, and operations boundary for the current pivot. Project is the canonical product and engineering noun. A Project belongs to an Organization and is owned by the Organization owner during the single-owner phase. Project has no runtime; Agents own Agent runtime, API endpoint exposure, and Threads / Sessions.
   ...
   - **Agent and resource ownership**: Agents, Threads / Sessions, Environments, Skills, MCP servers, Provider credentials, file records, Agent exposure state, Agent runtime logs/state, and project-scoped cost are Project-owned resources. The current UI keeps Agent logs/runtime operations on Agent detail and Project Usage under Project Settings; it does not expose a generic Project health/log console.
...
- **Cattle continuation has one committed boundary whether warm or cold**. After successful completion, Runtime excludes attachment mounts and transient credentials and creates a ready Sandbox Backup of the complete Session workspace. A follow-up cannot be admitted until that Run-bound backup is ready. Cold continuation restores the checkpoint and provider-native cursor before new input; warm continuation uses the equivalent resident state. If checkpoint creation fails, the previous ready checkpoint remains authoritative and the idle sweep cannot recycle the uncommitted subject; missing or corrupt restore state fails explicitly. Live processes, sockets, kernels, and machine-wide temporary state are not persisted.
