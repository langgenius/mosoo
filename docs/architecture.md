...
The existing Session kernel owns execution for inline and saved-preset creation. Resolve the configuration source before admission, then freeze one execution plan. Model-provider references and file ownership stay Project-scoped. Inline creation must not create a synthetic Agent or use a Session ID as a fake Agent ID. Make the optional Agent association truthful in Session, Run, event, and Driver contracts; legacy non-null references remain unchanged. Frozen execution must not re-read mutable preset configuration on continuation. Legacy snapshots lacking frozen configuration retain explicit compatibility reads until their migration is verified.
