...
The later September 22 migration decision permits a separately reviewed cohort of published legacy Pet Sessions to become read-only when the owner and their business have no visible activity for 30 days. Reuse the existing terminated lifecycle and new-Session entry point, preserving history and saved files; full recovery of their old native context/workspace is excluded. Refresh eligibility before cutover, retain protection for active or uncertain peers, and keep production approval and rollback requirements. This is a one-time migration exception, not a recurring formal-Session expiry. See [Thread Lifecycle](./prd/session-lifecycle.md#inactive-published-legacy-sessions-unreleased-migration-only).
