...
The September 19 scope decision makes #582 a BYOK release: callers configure a Project provider and save a private Agent before invocation. Platform-owned model supply, setup-free managed selectors, recharge, and commercial billing move to independent #636 work. Existing per-turn budgets and usage remain part of the Session execution guard.
...
Session-owned Codex memory lives in the runtime home inside the checkpointed workspace. Provisioning preserves a restored directory instead of linking it to machine-wide `/workspace/memory`. A legacy memory symlink requires a verified conversion before isolated execution or checkpoint commit; the link alone cannot establish whether its external contents were empty or recoverable. This guard is a release precondition for affected existing Sessions, not an accepted migration outcome. Keep their original mappings and backups until the shared-memory ownership, isolated copy, and rollback have been verified.
