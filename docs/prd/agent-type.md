...
> **UI status (2026-06-24, creation flow)**: the `+ New agent` primary CTA no longer opens the previous **Name + Description + Runtime profile** create dialog. It now opens a **New Agent** dialog (`create-agent-launcher.tsx`) that collects a required agent name and a runtime selection before creating anything — the runtime defaults to one derived from the active App's configured Provider credentials (`runtime-default.ts` receives the App-scoped credentials loaded by `useVendorCredentialsQuery(appId)`), and submit is blocked until the name is non-empty. On submit it creates the Agent via the existing `createAgent` mutation and navigates straight to that Agent's **Preview** tab (`/agent/{id}?tab=preview`), where the owner edits and tests through the Preview form surface. The old **Agent Builder** system-assistant path has been removed (#113); there is no first-message intake, templates row, `Start from blank` link, or natural-language generation step in this flow — see [architecture.md §4.4 Configuration Assistance Boundary](../architecture.md). The Configure-page kind selector (Pet/Cattle), the kind lock at first Publish, and the Fork-for-kind-switch path described in the user journey below are unchanged; only the upstream "fill in Name / Description / Runtime profile" framing in the §"User journey" Create row is stale — the create dialog now collects a name and runtime but no description, so read that row through this banner.
...
| **Files**                  | Long-lived uploaded file / knowledge assets. Both Pet and Cattle can read Files scoped to the same App; content stored as Files persists across Sessions / Sandboxes.                                                                                                           |
| **Non-persistent state**   | Content outside Files follows sandbox semantics: for Pet it stays in the stable Sandbox; for Cattle it disappears when the session ends, unless explicitly persisted.                                                                                                          |
