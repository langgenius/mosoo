...
Hosting substrate: the legacy path provisions account-level Cloudflare Pages/Worker sibling resources (project, script, route, domain) per deployment. When the `MOSOO_WFP_DISPATCH_NAMESPACE` env var names a Workers for Platforms dispatch namespace, new deployments instead upload one User Worker per App into that namespace through the Cloudflare REST API (static plans become asset-only Workers; worker plans keep their module and env vars), and deletion also clears the namespace script. This is phase 1 of the WfP migration (issue #281): it changes where artifacts are hosted but not the D1 deployment/run/binding model or capability authorization. Until dispatcher eyeball routing (phase 2) ships, namespace-hosted deployments are not reachable at their planned URL, so the flag must stay unset outside experiments.
