...
- Admin: "I set `@example.com` as our primary domain. When a new colleague signs up with `alex@example.com`, can they see our Org directly?"
...
- **Email is the primary identity.** Signing in with Google as `alex@example.com` and signing in with Email OTP as `alex@example.com` land on the same Account; they **never** become two.
