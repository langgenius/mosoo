...
Session isolation replaces shared writable Agent machines. Before changing existing Cloud workloads, inventory shared state and admitted work, verify each Session's recovery lineage, and define a transition for files, memory, or process behavior that cannot be preserved automatically. Preserve history, artifacts, and delegated identity. An inert historical type column may remain; changing labels or starting empty workspaces is not a migration.
