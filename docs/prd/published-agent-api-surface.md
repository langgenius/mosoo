...
| **SessionFile**                 | A copy of material that a user attaches to a given thread; its lifecycle follows the thread. The API list is the single product truth; a Files Panel surface (currently not rendered in the agent chat — see `docs/prd/session-files.md`) consumes the same list when shown.                                                                                                                                                                                           |
