...
There is no Hibernate button. mosoo may automatically recycle an idle Assistant sandbox using the same checkpoint boundary as Recreate. Task Agent live sandboxes may remain warm briefly and are recycled when idle, but each successfully completed turn atomically checkpoints the Thread working directory and provider resume state first. This Thread-scoped durability is separate from Assistant-style shared Agent memory; see [Thread Continuation](./thread-continuation.md).
