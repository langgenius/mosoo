/**
 * Migration-only physical history for the retired Project Deployment product
 * (formerly App Deployment).
 *
 ...
 */
import type { AccountId, PlatformId, ProjectId, SessionId, SessionRunId } from "@mosoo/id";
...
export const retiredProjectDeploymentsStorage = sqliteTable(
  "project_deployment",
  {
  ...
  (table) => [
    ...
    uniqueIndex("project_deployment_active_subdomain_idx")
      .on(table.mosooSubdomain)
      .where(sql`${table.deletedAt} IS NULL`),
  ],
);

export const retiredProjectDeploymentRunsStorage = sqliteTable(
  ...
  (table) => [
    ...
    index("project_deployment_run_deployment_id_idx").on(table.deploymentId, table.id),
    uniqueIndex("project_deployment_run_active_project_idx")
      .on(table.projectId)
      .where(
...
export const retiredProjectDeploymentSecretsStorage = sqliteTable(
  ...
  (table) => [
    uniqueIndex("project_deployment_secret_project_name_idx").on(table.projectId, table.name),
    uniqueIndex("project_deployment_secret_vault_secret_idx").on(table.vaultSecretId),
  ],
);
...
export const retiredSessionRunsPhysicalStorage = defineSessionRunsTable({
  ...
  retiredBoundCapabilityProjectId: platformIdColumn<PlatformId>("bound_capability_project_id"),
