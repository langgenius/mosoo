...
const GENERATED_ICON_KEYS_PATH = join(PACKAGE_ROOT, "src/runtime-icon-keys.generated.ts");
...
// planned entries, mirroring getRuntimeIconKey's historical lookup order.
function renderGeneratedIconKeys(catalog: GeneratedCatalog): string {
  const iconKeys: Record<string, string> = {};

  for (const runtime of catalog.runtimeCatalog) {
    iconKeys[runtime.runtimeId] = runtime.display.iconKey;
  }

  for (const planned of catalog.plannedRuntimeDisplayCatalog) {
    iconKeys[planned.runtimeId] ??= planned.iconKey;
  }

  return `// Generated by pkgs/runtime-catalog/scripts/generate-runtime-catalog.ts.\n// Source: pkgs/runtime-catalog/catalog/runtime-catalog.jsonc\n// Do not edit this file by hand.\n\nexport const GENERATED_RUNTIME_ICON_KEYS: Record<string, string> = ${literal(iconKeys)};\n`;
}
...
function main(): void {
  const catalog = createGeneratedCatalog(parseSource());
  const outputs: Array<[string, string]> = [
    [GENERATED_PATH, renderGeneratedCatalog(catalog)],
    [GENERATED_ICON_KEYS_PATH, renderGeneratedIconKeys(catalog)],
  ];
  ...
  if (check) {
    for (const [path, source] of outputs) {
      ...
      if (current !== source) {
        fail(`${path} is out of date. Run bun scripts/generate-runtime-catalog.ts.`);
      }
    }
